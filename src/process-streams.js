...
var IN = {"placeholder": "<INPUT>"};
var OUT = {"placeholder": "<OUTPUT>"};
/**
    ...
    if (args) {
        newArgs = args.map(function (arg) {
            switch (arg) {
                case IN:
                    if (tmpIn) {
                        throw new Error("Only one argument may be used as stream input!");
                    }
                    tmpIn = tmp();
                    return tmpIn;
                case OUT:
                    if (tmpOut) {
                        throw new Error("Only one argument may be used as stream output!");
                    }
                    tmpOut = tmp();
                    return tmpOut;
                default:
...
function parseCommand(command) {
    var tmpIn = null;
    var tmpOut = null;
    command = command.replace(IN.placeholder, function () {
        return tmpIn = tmp()
    });
    if (command.indexOf(IN.placeholder) >= 0) {
        throw new Error("Only one argument may be used as stream input!");
    }

    command = command.replace(OUT.placeholder, function () {
        return tmpOut = tmp();
    });
    if (command.indexOf(OUT.placeholder) >= 0) {
        throw new Error("Only one argument may be used as stream output!");
    }
...
module.exports = {
    /**
     * Placeholder for the file argument that is read by the child process
     */
    IN: IN,
    /**
     * Placeholder for the file argument that is written to by the process
     */
    OUT: OUT,
    ...
    exec: function (command, options, callback) {
        ...
        return wrapProcess(parsed.in, parsed.out, function () {
            return cp.exec(parsed.command, options, callback);
        });
    },
